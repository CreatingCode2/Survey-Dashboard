...
function isNoiseBody(threadText) {
...
function processTicket(ticketId, dryRun) {
  ...
  var EXCLUDED_SENDER_EMAILS = [
    ...
    'christine.larry.infolead@gmail.com'                          // Spam sender (ticket 90426)
  ];
  ...
  var HARDCODED_IGNORED_TICKETS = [88131, 90745, 90746, 90757, 90760, 90761, 88778, 88997, 89395, 87300, 86863, 88551, 88552, 91175, 90658, 91404, 90639, 90426];
  ...
  var prompt = "You are a customer support intelligence agent. Analyze the following support ticket thread.\n\n" +
    ...
    "- issue_type: A short, specific 2-4 word phrase describing the exact type of issue (e.g., NCOA File Upload, Database Migration, Login Failure, SFTP Access). Be descriptive and specific so we can accurately analyze issue trends later. Use 'Geo Products Usage Reporting' ONLY when the PRIMARY PURPOSE of the ticket is a customer submitting their periodic geocoder/geopoints/Geodata usage count for compliance or billing reporting (example subject: 'GeoCoding Products - Quarterly Reporting Q2/2026'). This rule DOES NOT apply if: the ticket subject is about a technical error, a verification inquiry, a missing file, or any other support issue that happens to mention geodata, geocoding, or international addresses in passing. Do NOT use this category for CLEAN_Address, Banner, PeopleSoft, Colleague, or any ERP integration ticket. Otherwise, do not restrict yourself to a predefined list.\n" +
    ...
    "- integration: If the ticket is for a Data Enhancement Service, format as 'DES - [Service]' and use ONLY these specific sub-categories: NCOA, Phone Append, Email Append, Social Append, MCOA, Deceased Append, PCOA, Global Address Verification, Geocoding, GeoBasic, GeoData, GeoPoints, Demographic Data, RBDI, CCOA. If it involves an ERP or Integration, you MUST aggressively scan the conversation for specific modules or interfaces. SELF SERVICE MODULE RULES (critical — follow exactly): (1) The term 'ssb' ALWAYS and EXCLUSIVELY maps to 'Banner - Self Service', regardless of any other ERP mentioned. (2) For all other ERPs (PeopleSoft, Colleague, Advance, JD Edwards, etc.), the terms 'self service', 'ss', 'ss x.x.x', 'ss 9.x', or similar self-service version strings map to '[Base ERP] - Self Service'. (3) Do NOT use 'Banner - Self Service' unless the ticket explicitly mentions Banner or ssb. OTHER MODULE RULES: Look for 'SaaS', 'HCM', 'FIN', 'Finance', 'cs', 'Campus Solutions' (map to PeopleSoft - CS), 'Admin', 'Classic', 'Fluid', 'EDI'. Format exactly as '[Base ERP] - [Module]'. You may output sub-categories like 'PeopleSoft - Fluid' or 'PeopleSoft - Classic', but MUST enforce the strict acronyms 'CS', 'HCM', and 'FIN' (e.g. use 'PeopleSoft - CS', NOT 'PeopleSoft - Campus Solutions'). If SurveyDIG is mentioned, output 'SurveyDIG'. DO NOT output 'Text Connector' or 'Guild Core Engine'. Valid Base ERPs: Advance, Banner, PeopleSoft, Colleague, JD Edwards, Oracle EBS, Oracle Database, SurveyDIG, None. CRITICAL: 'Person Manager' is an Ellucian SaaS-hosted product. Any ticket mentioning 'Person Manager' must output integration = 'Banner - SaaS' (or 'Colleague - SaaS' if Colleague is mentioned). Never output 'Person Manager' as the integration value. CRITICAL: FTP, SFTP, and file processing are NOT integrations - use 'None' for those.\n" +
...
function batchProcessTickets(dryRun) {
  ...
  var startDateStr = props.getProperty('AI_Batch_StartDate');
  var endDateStr   = props.getProperty('AI_Batch_EndDate');
  var updatedSince;
  
  if (startDateStr) {
    updatedSince = new Date(startDateStr).toISOString();
  } else {
    var d = new Date();
